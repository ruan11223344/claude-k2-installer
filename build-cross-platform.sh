...
# Claude Code + K2 Installer 跨平台构建脚本
...
BUILD_FLAGS="-p $CORES -ldflags=\"-w -s\" -tags bundled"

# 根据当前系统构建
case "$OS" in
    Darwin)
        # macOS 构建
        echo -e "${GREEN}构建 macOS 版本（使用 $CORES 线程）...${NC}"
        mkdir -p build/macos
        
        if [ "$ARCH" = "arm64" ]; then
            echo -e "${BLUE}构建 Apple Silicon 版本...${NC}"
            go build $BUILD_FLAGS -o "build/macos/${APP_NAME}" .
        else
    ...
    Linux)
        # Linux 构建
        echo -e "${GREEN}构建 Linux 版本（使用 $CORES 线程）...${NC}"
        mkdir -p build/linux
        go build $BUILD_FLAGS -o "build/linux/${APP_NAME}" .
    ...
    MINGW*|MSYS*|CYGWIN*)
        # Windows 构建
        echo -e "${GREEN}构建 Windows 版本（使用 $CORES 线程）...${NC}"
        mkdir -p build/windows
        
        # Windows 特殊处理：添加 -H windowsgui 隐藏控制台窗口
        GOMAXPROCS=$CORES go build -p $CORES -ldflags="-H windowsgui -w -s" -tags bundled -o "build/windows/${APP_NAME}.exe" .
...
echo -e "${BLUE}使用了 $CORES 个线程进行并行编译${NC}"